...
local get_config_gid_offset = function(context)
    return tonumber(context.configuration.gid_offset or "100000")
end


local get_config_uid_offset = function(context)
    return tonumber(context.configuration.uid_offset or "100000")
end
...
local resolve_gid_default_group = function(context)
    ...
    local groups = context.api:list_groups({ filter = { ldapQuery = "(cn=" .. group_name .. ")" } })
    local group_id = nil
